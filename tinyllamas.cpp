...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " PROMPT OUT-TOKEN-COUNT"
                  << std::endl;
        return 1;
    }

    std::string model_bin = "model.ncnn.bin", model_param = "model.ncnn.param",
                tokenizer_path = "tokenizer.bin", prompt = argv[1];
