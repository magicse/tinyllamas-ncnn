...
def apply_rotary_emb(
    ...
    xq_imm = torch.stack([xq_out_r, xq_out_i], dim=-1)
    shape = xq_imm.shape
    xq_out = xq_imm.view(shape[0], shape[1], shape[2] * shape[3])
    xk_imm = torch.stack([xk_out_r, xk_out_i], dim=-1)
    xk_out = xk_imm.view(shape[0], shape[1], shape[2] * shape[3])
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        xk = repeat_kv(xk, self.n_rep)  # (seqlen, n_local_heads, head_dim)
        xv = repeat_kv(xv, self.n_rep)  # (seqlen, n_local_heads, head_dim)
        ...
        # print("xq, xk:", xq.shape, xk.shape)
        scores = torch.matmul(xq, xk.transpose(1, 2)) / math.sqrt(self.head_dim)
        assert hasattr(self, 'mask')
        scores = scores + self.mask   # (n_local_heads, seqlen, cache_len + seqlen)
        scores = F.softmax(scores, dim=-1)
        scores = self.attn_dropout(scores)
        output = torch.matmul(scores, xv)  # (n_local_heads, seqlen, head_dim)
...
class Transformer(nn.Module):
    ...
    def __init__(self, params: ModelArgs):
        ...
        self.tok_embeddings.weight = torch.nn.Parameter(self.output.weight.clone()) # https://paperswithcode.com/method/weight-tying
